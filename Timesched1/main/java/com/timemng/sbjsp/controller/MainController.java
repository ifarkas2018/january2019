...
public class MainController {
    ...
    public String indexhome( Model model ) {
    	...
    	model.addAttribute("logged_in", isLoggedin); // adding the attribute logged_in to the model 
        return "index"; // return the index.jsp
    }
    	...
    	try {
			...
			if (empLoginDAO.addToQueryStr( user_name, user_passw )) {
				...
				if (!(list.isEmpty())) {
				    ...
				    return "index"; // show the index.jsp
				} else {
    ...
    public String task_update(Model model, @RequestParam(value="taskId", required=true) String id) {  
    	...
    	EmpSchedTaskInfo task_info = empSchedTaskDAO.findTask(Long.valueOf(id));
    ...
    public String show_update_results( Model model, @RequestParam(value="task_id", required=true) String id, @RequestParam(value="task_name", required=true ) String enter_tname, // 
    	...
    	if (update_succ) {
    		...
    		model.addAttribute("logged_in", "true"); // adding to the model : the user is logged in
    	}
    	else {
